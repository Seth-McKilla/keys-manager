...
const BASE_KEY_PATH = path.join(
  __dirname,
  "../../../",
  process.env.BASE_KEY_PATH
);
...
async function getAccount(publicKey) {
  ...
  const stateRootHash = (await c.getLatestBlockInfo()).block.header
    .state_root_hash;
...
async function printAccount(account) {
  ...
  console.log(
    JSON.parse(JSON.stringify(await getAccount(account.publicKey), null, 2))
  );
